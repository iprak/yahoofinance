...
@pytest.mark.parametrize(
    ...
    [
        ...
        {CONF_SYMBOLS: ["xyz"], CONF_SCAN_INTERVAL: "a"},
        # less than MINIMUM_SCAN_INTERVAL
        {
            CONF_SYMBOLS: ["xyz"],
            CONF_SCAN_INTERVAL: MINIMUM_SCAN_INTERVAL + timedelta(seconds=-1),
        },
        {CONF_SYMBOLS: ["xyz"], CONF_SCAN_INTERVAL: None},
    ],
)
async def test_invalid_global_scan_interval(
    hass: HomeAssistant, domain_config, enable_custom_integrations: None
) -> None:
...
def test_convert_to_float(value, expected) -> None:
